...
fit_stan = function(form, df, name, mm=TRUE, iter=2000) {
    ...
    diagnostic_file = paste0('models/', test, hemi, suffix, '.blme.diag.csv')
    if (mm) {
    ...
    sink(paste0('models/', name, '.blme.summary.txt'))
    pr("Formula:", stdout())
    pr(form, stdout())
...
fit_lme4 = function(form, df, name, mm=TRUE) {
    ...
    sink(paste0('models/', name, '.lme.summary.txt'))
    pr("Formula:", stdout())
    pr(form, stdout())
    print(summary(m))
    sink()
...
compare_stan = function(m1, m0, name) {
    ...
    sum_path = paste0('tests/', name, '.bf.summary.txt')
...
compare_lme4 = function(m1, m0, name) {
    ...
    sum_path = paste0('tests/', name, '.lrt.summary.txt')
...
save_fn = function(m, name) {
    if (BAYES) {
        save(m, file=paste0('models/', name, '.blme.Rdata'))
    } else {
        save(m, file=paste0('models/', name, '.lme.Rdata'))
    }
}
...
paths = list(
    ...
    SENTvNONW_lang=c('../../data/fMRI_ToM/ToMshort_n149/langfROIs_langlocSN_n149_data.csv'),
    BELIvPHOT_lang=c('../../data/fMRI_ToM/ToMshort_n149/langfROIs_ToMshort_n149_data.csv'),
    MENTvPHYS_lang=c('../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'),
    TOMVvTOMN_lang=c(
        '../../data/fMRI_ToM/ToMshort_n149/langfROIs_ToMshort_n149_data.csv',
        '../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'
    ),
    SENTvBELI_lang=c(
        '../../data/fMRI_ToM/ToMshort_n149/langfROIs_langlocSN_n149_data.csv',
        '../../data/fMRI_ToM/ToMshort_n149/langfROIs_ToMshort_n149_data.csv'
    ),
    SENTvPHOT_lang=c(
        '../../data/fMRI_ToM/ToMshort_n149/langfROIs_langlocSN_n149_data.csv',
    ...
    SENTvMENT_lang=c(
        '../../data/fMRI_ToM/Cloudy_n48/langfROIs_langlocSN_n48_data.csv',
        '../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'
    ),
    SENTvPHYS_lang=c(
        '../../data/fMRI_ToM/Cloudy_n48/langfROIs_langlocSN_n48_data.csv',
    ...
    MENTvSOCL_lang=c('../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'),
    MENTvPAIN_lang=c('../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'),
    PHYSvPAIN_lang=c('../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'),
    PHYSvSOCL_lang=c('../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'),
    SOCLvPAIN_lang=c('../../data/fMRI_ToM/Cloudy_n48/langfROIs_Cloudy_n48_data.csv'),

    # ToM

    SENTvNONW_ToM =c('../../data/fMRI_ToM/ToMshort_n149/ToMfROIs_langlocSN_n149_data.csv'),
    BELIvPHOT_ToM=c('../../data/fMRI_ToM/ToMshort_n149/ToMfROIs_ToMshort_n149_data.csv'),
    MENTvPHYS_ToM=c('../../data/fMRI_ToM/Cloudy_n48/ToMfROIs_Cloudy_n48_data.csv'),
    MENTvSOCL_ToM=c('../../data/fMRI_ToM/Cloudy_n48/ToMfROIs_Cloudy_n48_data.csv'),
    MENTvPAIN_ToM=c('../../data/fMRI_ToM/Cloudy_n48/ToMfROIs_Cloudy_n48_data.csv'),
    PHYSvPAIN_ToM=c('../../data/fMRI_ToM/Cloudy_n48/ToMfROIs_Cloudy_n48_data.csv'),
    PHYSvSOCL_ToM=c('../../data/fMRI_ToM/Cloudy_n48/ToMfROIs_Cloudy_n48_data.csv'),
    SOCLvPAIN_ToM=c('../../data/fMRI_ToM/Cloudy_n48/ToMfROIs_Cloudy_n48_data.csv')
)
...
df = read.csv('tom_effects_ling_controls.csv', header=TRUE)
ling_vars = c(
    'nwords',
    'nsent',
    'noF',
    ...
    'totsurp',
    'dltcvm',
    'nwords.nsent.noF.unigramsurp.fwprob5surp.totsurp.dltcvm'
)

for (ling_var in ling_vars) {
    for (test_type in c('BvP', 'BvPdelta')) {
        col = paste0(test_type, '.', ling_var)
        if (test_type == 'BvP') {
            test_name = 'BELIvPHOT'
        } else {
            test_name = 'BELIvPHOTdelta'
        }
        test = paste0(test_name, '-', gsub('.', '-', ling_var, fixed=TRUE), '_')
        hemis = c('LH', 'RH', 'AngG', 'ToM')
        for (hemi in hemis) {
            m1 = NULL
            m0 = NULL
            for (suffix in c('', '_h0')) {
                name = paste0(test, hemi, suffix)
                if (hemi == 'LH') {
                    df_ = df[(df$ROI < 6) & (df$Network == 'lang'),]
                } else if (hemi == 'RH') {
                    df_ = df[(df$ROI > 6) & (df$ROI < 12) & (df$Network == 'lang'),]
                } else if (hemi == 'AngG') {
                    df_ = df[((df$ROI == 6) | (df$ROI == 12)) & (df$Network == 'lang'),]
                } else if (hemi == 'ToM') {
                    df_ = df[df$Network == 'ToM',]
                } else {
                    stop(paste0('Unrecognized network name: ', hemi))
                }
                if (suffix == '_h0') {
                    f = paste0(col, ' ~ 0 + (1 | Subject) + (1 | ROI)')
                } else {
                    f = paste0(col, ' ~ (1 | Subject) + (1 | ROI)')
                }
